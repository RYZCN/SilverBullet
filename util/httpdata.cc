...
namespace SB
{
    namespace util
    {
        ...
        const __uint32_t DEFAULT_EVENT = EPOLLIN | EPOLLET | EPOLLONESHOT; //保证一个线程读取
        ...
        HttpMessage::HttpMessage(EventLoop *loop, int connfd)
            : loop_(loop),
              ...
              keep_alive_(false)
        {
            channel_->set_read_handler(bind(&HttpMessage::handle_read, this));
            channel_->set_write_handler(bind(&HttpMessage::handle_write, this));
            channel_->set_env_handler(bind(&HttpMessage::handle_env, this));
        }
        ...
        void HttpMessage::seperateTimer()
        {
        ...
        void HttpMessage::handle_read()
        {
            __uint32_t &events_ = channel_->get_events();
            do
            {
                bool zero = false;
                int read_num = readn(fd_, in_buff_, zero);
                LOG << "Request: " << in_buff_;
                ...
                if (read_num < 0)
                {
                    perror("1");
                    error_ = true;
                    handle_err_(fd_, 400, "Bad Request");
                    break;
                }
                else if (zero)
                {
                ...
                if (state_ == STATE_RECV_BODY)
                {
                    ...
                    {
                        error_ = true;
                        handle_err_(fd_, 400, "Bad Request: Lack of argument (Content-length)");
                        break;
                    }
                    if (static_cast<int>(in_buff_.size()) < content_length)
                        break;
                    state_ = STATE_ANALYSIS;
                }
        ...
        void HttpMessage::handle_write()
        {
            if (!error_ && connection_state_ != H_DISCONNECTED)
            {
                __uint32_t &events_ = channel_->get_events();
                if (writen(fd_, out_buff_) < 0)
                {
                    perror("writen");
                    events_ = 0;
                    error_ = true;
        ...
        void HttpMessage::handle_env()
        {
            ...
            {
                loop_->add_loop_task(bind(&HttpMessage::handle_close, shared_from_this()));
            }
        }

        URIState HttpMessage::parse_uri()
        {
            string &str = in_buff_;
            string cop = str;
            // 读到完整的请求行再开始解析请求
            size_t pos = str.find('\r', read_pos_);
            if (pos < 0)
            {
        ...
        HeaderState HttpMessage::parse_headers()
        {
            string &str = in_buff_;
        ...
        AnalysisState HttpMessage::analysis_request()
        {
            if (method_ == METHOD_POST)
            {
            }
            else if (method_ == METHOD_GET || method_ == METHOD_HEAD)
            {
                ...
                if (file_name_ == "favicon.ico")
                {
                    ...
                    header += "\r\n";
                    out_buff_ += header;
                    out_buff_ += string(favicon, favicon + sizeof favicon);
                    ;
                    return ANALYSIS_SUCCESS;
                }
        ...
        void HttpMessage::handle_err_(int fd, int err_num, string short_msg)
        {
            short_msg = " " + short_msg;
            char send_buff[4096];
